#!/usr/bin/env python3
"""
US Resume Personal Assistant - 集成测试脚本（修复版）
测试Chrome扩展的所有模块和功能
"""
...
def check_js_syntax(content):
    ...
    open_paren = cleaned_content.count('(')
    close_paren = cleaned_content.count(')')
    if abs(open_paren - close_paren) > 2:
        return False
    
    open_bracket = cleaned_content.count('[')
    close_bracket = cleaned_content.count(']')
    if abs(open_bracket - close_bracket) > 2:
        return False
...
def check_communication_api(content):
    # 检查chrome.runtime API或自定义sendMessage方法
    return ('chrome.runtime.sendMessage' in content or 
            'chrome.runtime.onMessage' in content or
            'sendMessage(' in content or
            'async sendMessage' in content)
...
def main():
    log_header('US Resume Personal Assistant - 集成测试（修复版）')
    ...
    test_file_content('src/content/enhanced-linkedin-monitor.js', 'LinkedIn监控模块结构',
                     lambda c: check_module_structure(c, ['class EnhancedLinkedInMonitor', 'observeJobListings', 'extractJobDetails']))
    ...
    for file_path, api in modules:
        if os.path.exists(file_path):
            if api == '通信接口':
                test_file_content(file_path, f"{file_path} 包含通信接口", check_communication_api)
            else:
    ...
    # 6.1 检查API密钥安全处理
    test_file_content('popup/index.html', 'Popup包含API密钥安全输入', check_security_features)
